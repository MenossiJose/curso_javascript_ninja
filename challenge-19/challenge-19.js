...
function hasClass (markup, cssClass) {
	...
	var regex = new RegExp('class=[\\"\']?\'?(?:\w+)\s?(?:\w+)["\\]?\'?', 'gm' );
	return regex.text(markup);
}

var classes = ['container', 'text', 'date', 'excerpt', 'main'];

classes.forEach(function(cssClass){
	console.log(hasClass(markup, cssClass) + ' para a classe ' + item);
}); */

const regex = /class=[\\"']?'?(?:\w+)\s?(?:\w+)["\\]?'?/gm;

// Alternative syntax using RegExp constructor
// const regex = new RegExp('class=[\\\\"\']?\'?(?:\\w+)\\s?(?:\\w+)["\\\\]?\'?', 'gm')
